...
app.post('/api/timetable', async (req, res) => {
  ...
  const { classId, day, time, period, subject, teacher } = req.body;

  const timetable = await TimetableModel.findOne({ classId: new Types.ObjectId(classId) });
  ...
  if(timetable){
    ...
    return res.json({ message: "✅ Timetable entry added", timetable });

  } else {
    const newPeriod = new TimetableModel({
    ...
    res.json({ message: "✅ Timetable entry added", newPeriod });
  }
});
...
app.get('/api/timetable/:classId', async (req, res) => {
  ...
  const timetable = await TimetableModel.findOne({classId: new Types.ObjectId(classId)
  }).populate('entries.subject').populate('entries.teacher');
...
app.put('/api/timetable/:id', async (req, res) => {
  ...
  const updatedEntry = await TimetableModel.findOneAndUpdate(entryId, req.body, { new: true });
